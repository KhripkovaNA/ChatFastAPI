...
const sendRequest = async (url, formType, data) => {
    try {
        ...
        if (response.ok) {
            ...
            if (formType === 'register') {
                alert('Вы зарегестрированы, теперь залогинтесь!');
                showTab("login")
            }
            return result;
        } else {
...
// Функция для определения Telegram ID из адресной строки
const getTelegramID = () => {
    let address = new URL(window.location);
    let queryParameters = address.searchParams;
    let tg_id = queryParameters.get("tg_id");
    return tg_id;
    }
...
const handleFormSubmit = async (formType, url, fields) => {
    ...
    const data = await sendRequest(url, formType,  formType === 'login'
        ? {email: fields[0], password: fields[1], tg_id: getTelegramID()}
        : {email: fields[0], name: fields[1], password: fields[2], password_check: fields[3], tg_id: getTelegramID()});
